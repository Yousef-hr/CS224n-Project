"""
JEPA MoE + SIGReg text classifier. Encoder owned by runner.
"""
...
class MoESigRegJEPATextClassifier(BaseTextClassificationModel):
    def __init__(
        self,
        embed_dim: int,
    ...
    def loss(
        ...
        if ctx is None or ctx.label_embeddings is None:
            raise ValueError("MoESigRegJEPATextClassifier requires ctx.label_embeddings for loss()")
    ...
    def scores(self, outputs: torch.Tensor, ctx: RunContext | None = None) -> torch.Tensor:
        if ctx is None or ctx.label_embeddings is None:
            raise ValueError("MoESigRegJEPATextClassifier requires ctx.label_embeddings for scores()")
