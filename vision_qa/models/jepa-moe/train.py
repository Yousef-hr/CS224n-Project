"""Train JEPA MoE VQA (frozen CLIP + projection + MoE predictor).
...
from .model import VisionQAJEPAMoE
...
@torch.no_grad()
def _moe_metrics(model, loader, ctx) -> dict[str, float]:
    model.eval_mode()
...
def main() -> None:
    ...
    train_spec = TrainSpec(
        ...
        metrics_csv=args.metrics_csv or str(Path(args.save_dir) / "training_metrics_jepa_moe.csv"),
    )
    ...
    if args.embedding_cache_dir:
        prepare_and_run_cached(
            ...
            extra_eval_metrics=_moe_metrics_cached,
        ...
        run_vision_qa_train(
            ...
            extra_eval_metrics=_moe_metrics,
            save_name="best_jepa_moe.pt",
        )
