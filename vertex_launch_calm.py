...
def main():
    ...
    ae_job = run_job(args, f"{args.run_name}-ae", ae_command, common_env, sync=True)
    ...
    energy_job = run_job(args, f"{args.run_name}-energy", energy_command, post_ae_env, sync=False)
    ...
    jepa_job = run_job(args, f"{args.run_name}-jepa", jepa_command, post_ae_env, sync=False)
