...
def full_evaluation(model, loader, device, label_embeddings):
    """Single-pass eval: loss, accuracy, and representation metrics."""
    model.eval()
    total_loss = 0.0
    correct = 0
    n = 0
...
def main():
    ...
    metrics_rows = []
    metrics_csv_path = Path(args.metrics_csv) if args.metrics_csv else save_dir / "training_metrics_baseline.csv"
    ...
    for ep in range(args.epochs):
        ...
        print(f"Epoch {ep + 1}/{args.epochs} | train_loss={loss:.4f} | train_sigreg={train_sigreg:.4f}")
        metrics_rows.append({"epoch": ep + 1, "train_loss": loss, "train_sigreg": train_sigreg})

    eval_results = full_evaluation(model, test_loader, device, label_embeddings)
    metrics_rows[-1].update(eval_results)
    save_checkpoint(save_dir / "best_jepa_baseline.pt", model=model, optimizer=optimizer, epoch=args.epochs, eval_acc=eval_results["eval_acc"])
    print(
        f"Final eval | eval_loss={eval_results['eval_loss']:.4f} | eval_acc={eval_results['eval_acc']:.4f}"
    )
    print(
        f"  repr: pred_erank={eval_results['pred_effective_rank']:.4f} "
        f"| target_erank={eval_results['target_effective_rank']:.4f} "
